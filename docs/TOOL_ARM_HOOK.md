...
- **Least privilege.** The hook is for hardware/GUI delegation — not a general remote
  shell for arbitrary callers. Only the FULL node holds the secret.
- **Auditable.** Log every call; the FULL node's checkpoints reference what it asked the
  Tool Node to do.

## Lifecycle

Run the hook under a process manager (launchd / systemd / equivalent) so it restarts on
boot and on crash. Health-check it from the FULL node before delegating a task that
depends on hardware.
