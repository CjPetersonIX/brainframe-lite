...
A small, always-on local machine that gives the headless cloud FULL node a body:

| Capability | Why the cloud can't do it itself |
|---|---|
| GUI / desktop automation | The FULL node is headless — no display server. |
...
| **Floor** | **as low as 4 GB** | **Runs — but expect some lag.** Keep one workload in memory, lean on the FULL node swarm for cognition. |
...
6. **Offload cognition.** Heavy reasoning/codegen belongs on the FULL node swarm — call
...
- It never becomes a single point of failure for the whole stack — if it's down, the
  FULL node keeps running and simply loses hardware reach until it returns.
